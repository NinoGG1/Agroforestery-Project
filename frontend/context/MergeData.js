...
export const MergeDataProvider = ({ children }) => {
  ...
  // Récupérer les métadonnées pour les CIDs des événements SFT1 et SFT2
  useEffect(() => {
    const cids = [...mergedSft1Events, ...mergedSft2Events].map(
      (event) => event.cid
    );
    fetchMetadata(cids);
  }, [mergedSft1Events, mergedSft2Events]);

  // Fusionner les événements SFT1 et SFT2 avec les métadonnées correspondantes
  useEffect(() => {
    const mergeEventsAndMetadata = () => {
      const merged = [...mergedSft1Events, ...mergedSft2Events].map((event) => {
  ...
  }, [metadata, mergedSft1Events, mergedSft2Events]);

  useEffect(() => {
    console.log("Merged SFT1 and SFT2 Events and Metadata:", sfts);
  }, [sfts]);
